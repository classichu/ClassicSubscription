package com.classichu.subscription.adapter;
...
import com.classichu.subscription.R;
...
public class SubscriptionRecyclerViewAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public final static int VIEW_TYPE_MY_TITLE = 121;
    public final static int VIEW_TYPE_MY_LIST = 122;
    ...
    private int findMoreTitleItemIndex() {
         ...
         */
        for (int i = 0; i < mSubscriptionBeanList.size(); i++) {
            if (mSubscriptionBeanList.get(i).getViewType() == VIEW_TYPE_MORE_TITLE) {
                pos = i;
                break;
            }
        }
        CLog.d("MoreTitleItemIndex:" + pos);
        return pos;
    }
            ...
            viewHolder.itemView.setOnClickListener(new OnNotFastClickListener() {
                ...
                protected void onNotFastClick(View v) {
                    ...
                    if (clickPosition > moreTitleItemIndex) {//大于moreTitleItemIndex  支持点击订阅新的channel
                        ...
                        newPosition = mSubscriptionBeanList.size() - 1;
                    }
                    if (clickPosition < newPosition) {
                        for (int i = clickPosition; i < newPosition; i++) {
                            Collections.swap(mSubscriptionBeanList, i, i + 1);
                        }
                    } else {
                        for (int i = clickPosition; i > newPosition; i--) {
                            Collections.swap(mSubscriptionBeanList, i, i - 1);
    ...
    public int getItemCount() {
        return mSubscriptionBeanList.size();
    }

    @Override
    public int getItemViewType(int position) {
        //return super.getItemViewType(position);
        return mSubscriptionBeanList.get(position).getViewType();
    }
       ...
       */
        ...
        public void onSelectedChanged(RecyclerView.ViewHolder viewHolder, int actionState) {
            ...
            if (actionState != ItemTouchHelper.ACTION_STATE_IDLE) {
                viewHolder.itemView.setBackgroundColor(ContextCompat.getColor(viewHolder.itemView.getContext(),R.color.classicGridItemSelectedBg));
            }
        }
